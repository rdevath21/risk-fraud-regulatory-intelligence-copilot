...
The project includes an MCP (Model Context Protocol) plugin that extends CoCo with filesystem access for investigation report export:

**Plugin:** `mcp/plugin.json`

```json
{
  "mcpServers": {
    "risk-investigation-files": {
      "command": "npx",
      "args": ["-y", "@anthropic-ai/mcp-filesystem@latest", "<PATH>/reports"]
    }
  }
}
```
...
**What it enables:**
- CoCo can read/write investigation evidence reports to the `reports/` directory
- AI-generated evidence memos are exported as structured text files for compliance archival
- Turns read-only AI analysis into cross-tool action (CoCo generates report in Snowflake, exports via MCP to filesystem)

**Sample output:** `reports/evidence_ALT-001.txt` -- a full evidence report generated by the copilot

**To install the plugin in CoCo Desktop:**
1. Copy the `mcp/` folder to `~/.snowflake/cortex/plugins/risk-copilot-mcp/.cortex-plugin/`
2. Rename `mcp/plugin.json` to `.cortex-plugin/plugin.json`
3. Restart CoCo Desktop
...
|
|-- mcp/
|   |-- plugin.json               # MCP plugin config for CoCo Desktop
|
|-- reports/
...
| **MCP connectors** | Filesystem MCP plugin for investigation report export to local files |
